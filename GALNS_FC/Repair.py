...
class Repair():
    ...
    def run_ga_idle(self, elements, start_node, end_node, population_size, generations, mutation_rate):
        # 유전 알고리즘 실행
            ...
            while j < len(route):
                ...
                if route[j][1] == IDLE or route[j][1] == CATCH:
                    ...
                    if end_index is not None:
                        ...
                        if len(only_idles) >= 2:
                            best_idles = self.run_ga_idle(only_idles, start_node, end_node, population_size, generations, mutation_rate)
                        else:
    ...
    def temp_kwh_km(self,efficiency, temperature,velocity):
        consumption_efficiency = (1 + (0.12222619026903651*temperature*temperature - 5.937023805626322*temperature + 86.24285720745925)/100)
        recovered_efficiency =(1 - (-0.03798796799702986*temperature*temperature + 1.6493582896681127*temperature + 14.121804337124948)/100)
        velocity_efficiency= (1 + (18.158241873118623*velocity*velocity - 23.115165145819724*velocity + 13.325274888899935)/100)
        
        if (0.12222619026903651*temperature*temperature - 5.937023805626322*temperature + 86.24285720745925) < 0:
            consumption_efficiency = 1
        if (-0.03798796799702986*temperature*temperature + 1.6493582896681127*temperature + 14.121804337124948) < 0:
            recovered_efficiency = 1
        if (1 + (18.158241873118623*velocity*velocity - 23.115165145819724*velocity + 13.325274888899935)/100) < 0:
            velocity_efficiency = 1

        energy_efficiency = efficiency * consumption_efficiency * recovered_efficiency * velocity_efficiency
    ...
    def drone_k_opt_distance(self, route):
        distance = 0
        for i in range(len(route) - 1):
            edge_weight = (data["edge_km_d"][route[i][0]][route[i+1][0]]*data["curvy_air"])
            distance += edge_weight
        return distance
    ...
    def truck_k_opt_distance(self, route):
        distance = 0
        for i in range(len(route) - 1):
            edge_weight = (data["edge_km_t"][route[i][0]][route[i+1][0]]*data["curvy_road"]) 
            distance += edge_weight
        return distance
            ...
            while j < len(route):
                if route[j][1] == FLY:
                    ...
                    if end_index is not None:  
                        ...
                        if len(only_drones) > 1:
                            best_drone_distance = float('inf')
                            best_drone_perm = None
                            drone_perms = [list(perm) for perm in itertools.permutations(only_drones)]

                            for perm in drone_perms:
                                ...
                                if distance < best_drone_distance:
                                    best_perm = perm
                                    best_drone_distance = distance
                        ...
                        if len(only_trucks) > 1:
                            best_truck_distance = float('inf')
                            best_truck_perm = None
                            truck_perms = [list(perm) for perm in itertools.permutations(only_trucks)]

                            for perm in truck_perms:
                                path = [(route[start_index][0], route[start_index][1])] + [(truck, ONLY_TRUCK) for truck in perm] + [(route[end_index][0], route[end_index][1])]
                                distance = self.truck_k_opt_distance(path)
                        
                                if distance < best_truck_distance:
                                    best_perm = perm
                                    best_truck_distance = distance
                        ...
                        j = end_index + 1
                        continue  # Skip the remaining code in this loop and continue with the next iteration
                    else:
                        j += 1  # Increment j if end_index is not found
                else:
                    j += 1  # Increment j if route[j][1] is not IDLE or CATCH
    ...
    def idle_consumption(self,efficiency,time):
        consumption_increase = (1 + (0.12222619026903651*time*time - 5.937023805626322*time + 86.24285720745925)/100)
        consumption = efficiency*consumption_increase
        return consumption
    ...
    def soc_calculate(self, route):
        ...
        for idx in range(1, len(route)):
            if route[idx][1] == CATCH or ((route[idx][1] == FLY) and (route[idx-1][1] in [ONLY_TRUCK, ONLY_DRONE])):
                ...
                truck_energy_consumption = (data["edge_km_t"][route[idx-jump-1][0]][route[idx][0]]*data["curvy_road"]) * self.temp_kwh_km(data["energy_kwh/km_t"],data["temperature"],data["speed_t"])
                truck_energy_consumption += self.idle_consumption(data["idle_t"], (data["service_time"]/60))
                #이거 웨이팅 타임 해줘야함
