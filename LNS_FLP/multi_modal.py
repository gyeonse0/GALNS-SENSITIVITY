...
vrp_file_path = r'C:\Users\User\OneDrive\바탕 화면\realrealrealreal-main\realrealrealreal-main\LNS_FC\data\multi_modal_data.vrp'
backup_file_path = r'C:\Users\User\OneDrive\바탕 화면\realrealrealreal-main\realrealrealreal-main\LNS_FC\data\multi_modal_data_backup.vrp'
...
class RouletteWheel:
    ...
    def select_operators(self):
        selected_index = np.random.choice(len(self.operators))
    ...
    if current_num==0:
        ...
        if np.exp((MultiModalState(current_states[-1]).cost_objective() - MultiModalState(repaired_state).cost_objective()) / temperature) >= rnd.random():
            ...
            if objective_value == min(objectives):
                outcome = 1
                min_objective_time=time.time()
                k_opt_state = Rep.drone_k_opt(repaired_state,rnd_state)
